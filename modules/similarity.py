"""Chemical Similarity Search – Tanimoto / Morgan fingerprint search."""
...
from utils.chem_utils import (
    ...
    draw_molecule_png_b64, RDKIT_AVAILABLE,
)


def render():
    st.markdown("<div class='section-head'>🔍 Chemical Similarity Search</div>", unsafe_allow_html=True)

    st.markdown("""
    <div class='info-box'>
    Uses <b>Morgan fingerprints</b> (radius=2, 2048 bits) and <b>Tanimoto coefficient</b>
    to find structurally similar VOCs. A score of 1.0 = identical; &gt;0.4 = structurally similar.
    </div>
    ...
    with tab1:
        ...
        with col_a:
            mode = st.radio("Query by", ["Select from Library", "Enter SMILES"], horizontal=True)
        with col_b:
        ...
        if query_smiles and st.button("🔍 Search", type="primary"):
            ...
            if results_df.empty:
                ...
                for col, (_, row) in zip(cols, top4.iterrows()):
                    with col:
                        b64 = draw_molecule_png_b64(row["SMILES"], 240, 160)
                        if b64:
                            st.markdown(
                        ...
                        bio_c = "#22c55e" if row["Bioactivity"] == "Attractant" else "#ef4444"
                        st.markdown(
                            f"**{row['VOC']}**  \n"
                ...
                display = results_df.drop(columns=["SMILES"]).copy()
                display["Similarity"] = display["Similarity"].apply(lambda x: f"{x:.4f}")
                st.dataframe(
                    display.style.applymap(color_bio, subset=["Bioactivity"]),
    ...
    with tab2:
        ...
        with st.spinner("Computing full similarity matrix..."):
            ...
            names = [n[:22] + "…" if len(n) > 22 else n for n in VOCS["name"].tolist()]
        ...
        **Interpretation guide**
        - **1.00** = Identical structure
        - **0.85+** = Very similar (same scaffold)
        - **0.40–0.85** = Structurally related
        - **< 0.40** = Structurally distinct
        """)
